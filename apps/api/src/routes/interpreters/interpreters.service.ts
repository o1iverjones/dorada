...
export async function getInterpreterActivity(id: string, organizationId: string, prisma: PrismaClient) {
  const interpreter = await prisma.interpreter.findUnique({ where: { id }, select: { organization_id: true } });
  ensureTenant(interpreter, organizationId, "INTERPRETER_NOT_FOUND");
  return prisma.activityLog.findMany({
    where: { entity_type: "interpreter", entity_id: id, organization_id: organizationId },
    orderBy: { created_at: "desc" },
  });
}

export async function getInterpreterNotes(id: string, organizationId: string, prisma: PrismaClient) {
  const interpreter = await prisma.interpreter.findUnique({ where: { id }, select: { organization_id: true } });
  ensureTenant(interpreter, organizationId, "INTERPRETER_NOT_FOUND");
  return prisma.interpreterNote.findMany({
    where: { interpreter_id: id },
    orderBy: { created_at: "desc" },
  });
}

export async function addInterpreterNote(
  id: string,
  content: string,
  organizationId: string,
  ...
  const interpreter = await prisma.interpreter.findUnique({ where: { id }, select: { organization_id: true } });
  ensureTenant(interpreter, organizationId, "INTERPRETER_NOT_FOUND");
  return prisma.interpreterNote.create({
    data: {
      interpreter_id: id,
      organization_id: organizationId,
      content,

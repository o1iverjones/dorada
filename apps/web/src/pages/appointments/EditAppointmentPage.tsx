...
export function EditAppointmentPage() {
  ...
  return (
    <div>
      <PageHeader title={t("appointments.edit_title")} />
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="col-span-full flex justify-end gap-2 pt-2">
              <Button type="button" variant="outline" onClick={() => navigate(`/appointments/${id}`)}>{t("common.cancel")}</Button>
              <Button type="submit" disabled={isSubmitting}>{isSubmitting ? t("common.saving") : t("common.save_changes")}</Button>
            </div>
          </form>
        </CardContent>
      </Card>

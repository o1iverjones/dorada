...
export function useReactivateInterpreter(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () => api.post(`/interpreters/${id}/reactivate`, {}),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["interpreters", id] });
      qc.invalidateQueries({ queryKey: ["interpreters"] });
    },
...
export function useInterpreterActivity(id: string) {
  return useQuery({
    queryKey: ["interpreters", id, "activity"],
    queryFn: () => api.get<unknown[]>(`/interpreters/${id}/activity`),
    enabled: !!id,
  });
}

export function useInterpreterNotes(id: string) {
  return useQuery({
    queryKey: ["interpreters", id, "notes"],
    queryFn: () => api.get<unknown[]>(`/interpreters/${id}/admin-notes`),
    enabled: !!id,
  });
}

export function useAddInterpreterNote(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ content, image_url }: { content: string; image_url?: string | null }) =>
      api.post(`/interpreters/${id}/admin-notes`, { content, image_url }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["interpreters", id, "notes"] }),
  });
}

export function useUploadInterpreterNoteImage(id: string) {
  return useMutation({
    mutationFn: (file: File) => {
      const fd = new FormData();
      fd.append("file", file);
      return api.uploadFile<{ url: string }>(`/interpreters/${id}/note-image`, fd);
    },
  });
}
